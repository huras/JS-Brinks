...
class Vector2 {
    ...
    clamp(max){
        if(Math.abs(this.x) > max.x)
            this.x = max.x * Math.sign(this.x);
        
        if(Math.abs(this.y) > max.y)
            this.y = max.y * Math.sign(this.y);
    }
...
class InputManager {
    constructor(keys = {right: 39, left: 37, up: 38, down: 40, run: 88, jump: 67}){
        this.keys = keys;
        ...
        setInterval(() => { this.timeCounter(10) }, 10);
    }
...
// ========================================= Sprites & Animations & Sheets & Tiles

class SpriteCell {
    constructor(x, y, width, height, sheet, delay = 10){
        this.x = x; this.y = y; this.width = width; this.height = height; this.sheet = sheet; this.delay = delay;
    }
}
    ...
    render(position, direction){
        if(this.currentAnimation && this.isPlaying){
            ...
            if(this.lastPosition){
                // Clear the canvas
                this.context.clearRect(this.lastPosition.x - currentCell.width/2, this.lastPosition.y - this.lastSize.height , this.lastSize.width, this.lastSize.height);
            }
            ...
            currentCell.width,
            currentCell.height,
            position.x * -direction - currentCell.width / 2, //destination x
            position.y - currentCell.height, //destination y
            currentCell.width, //destination width
            currentCell.height); //destination height
...
class SheetLoader {
    ...
    loadSheet(filepath){
        ...
        newSheet.addEventListener("load", () => { this.onLoadSheet() });
...
class Character {
    ...
    control(){
        ...
        if(this.inputManager.isPressed(this.keys.down)) {
            ...
            if(this.inputManager.isPressed(this.keys.left) || this.inputManager.isPressed(this.keys.right)){
                let speedFactor = 1;
...
class Engine{
    ...
    teste1(){
        let callBack = () => {
            ...
            let animations = [marioIdle, marioWalk, marioLookup, marioCrouch, marioSliping, marioJumpRise, marioJumpFall];
    
            let spritePlayer = new SpritePlayer(animations, this.context, 4);

            let mario = new Character(spritePlayer, new Vector2(20,32));
